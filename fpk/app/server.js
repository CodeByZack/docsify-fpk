 ...
 *  - 两边拖 .md / .html / .htm 都会自动出现在侧边栏（html 作为独立页面伺服）
 *
 * 特性：
 ...
 */
const SIDEBAR_SKIP = new Set(['README.md', 'index.html', '_sidebar.md', '_coverpage.md', '_navbar.md', '_footer.md', '_404.md']);
...
/** 递归收集目录下所有 .md / .html / .htm，返回相对路径（如 sub/deep.md），按名称排序 */
function listMarkdownFiles(root) {
  ...
  (function walk(dir, rel) {
    ...
    entries.forEach((ent) => {
      ...
      } else if ((name.endsWith('.md') || name.endsWith('.html') || name.endsWith('.htm')) && !SIDEBAR_SKIP.has(name)) {
        out.push(relPath);
      }
    });
  })(root, '');
